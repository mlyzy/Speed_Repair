...
@register_criterion("narrepair_loss")
class LabelSmoothedDualImitationCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample.get("cmlm_prev_output_tokens") is not None:
            cmlm_prev_output_tokens = sample["cmlm_prev_output_tokens"]
        else:
            cmlm_prev_output_tokens = None
        if "glat" in sample:
            glat = sample["glat"]
        else:
            glat = None
        ...
        # ipdb.set_trace()
        treepath = sample["treepath"]
        # outputs = model(src_tokens, src_lengths, prev_output_tokens, cmlm_prev_output_tokens, tgt_tokens, glat ,treepath)
        outputs = model(
            src_tokens,
            src_lengths,
            prev_output_tokens,
...
def log_metric(key, logging_outputs):
    if len(logging_outputs) > 0 and key in logging_outputs[0]:
        metrics.log_scalar(
            key,
            utils.item(np.mean([log.get(key, 0) for log in logging_outputs])),
            priority=10,
            round=3,
        )
