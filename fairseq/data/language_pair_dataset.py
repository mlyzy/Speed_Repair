...
class LanguagePairDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.src_with_werdur:
            ...
            # ipdb.set_trace()
            pathlength = treepath[-1]
            treepath = treepath[:-1]
            pathlength = pathlength[~np.isin(pathlength, [-1])]
            treepath = treepath.reshape(-1)
            treepath = treepath[~np.isin(treepath, [-1])].numpy()
            newtreepath = np.full((len(pathlength), 50), -1)
            pathindex = 0
            # ipdb.set_trace()
            for i in range(len(pathlength)):
                newtreepath[i][-1 * pathlength[i] :] = treepath[
                    pathindex : pathindex + pathlength[i]
                ]
                pathindex = pathindex + pathlength[i]
            tgt_item = tgt_item[: nodelenth + 1]
            ast_code_links = np.zeros((newtreepath.shape[0], newtreepath.shape[0]))
            # ipdb.set_trace()
