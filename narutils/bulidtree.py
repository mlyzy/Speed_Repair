...
def bulid_tree(javacode):
    code_tokens, ast_leaves, ast_leaf_ranges = add_structure(javacode)
    # id_tokens = tokenize_codes_texts(javacode)
    # code_ranges = get_code_tokens_ranges(javacode,id_tokens)
    ...
    # print(lr_paths)
    ast_paths = get_token_patchs(javacode, code_tokens, lr_paths)
    # return lr_paths,ast_leaf_code_token_idxs
    return ast_paths


def get_token_patchs(javacode, code_tokens, lr_paths):
    java_tokens = javacode.split()
    for i in range(
        len(code_tokens) - 1, -1, -1
    ...
    for token in code_tokens:
        ...
        if tokenlink == java_tokens[num]:
            tokenlink = ""
            num = num + 1
            index.append([start, end])
            start = end
    for maxast in index:
        new_lr_paths.append(max(lr_paths[maxast[0] : maxast[1]], key=len))
    return new_lr_paths
...
def add_structure(code, lang="java"):
    LANGUAGE = Language(
        "/mnt/hdd/yzy/yangzhenyu/FastCorrect/FC_utils/parser/my-languages2.so",
        "c_sharp" if lang == "cs" else lang,
    )
    parser = Parser()
...
def tokenize_codes_texts(texts):
    tokenizer = RobertaTokenizer.from_pretrained(
        "/mnt/hdd/yzy/yangzhenyu/pretrain_model/codegraphbert/"
    )
