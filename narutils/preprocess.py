...
def main(args):
    ...
    task = tasks.get_task(args.task)#translation为task
    ...
    if args.joined_dictionary:
        ...
        if args.srcdict:
            src_dict = task.load_dictionary(args.srcdict)  # 加载源语言字典
        else:
            assert (
                args.trainpref
            ), "--trainpref must be set if --srcdict is not specified"
        ...
        if target:
            if args.tgtdict:
                tgt_dict = task.load_dictionary(args.tgtdict)  # 加载目标语言字典
            else:
                assert (
                    args.trainpref
    ...
    src_dict.save(dict_path(args.source_lang))  # 保存字典文件
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        pool = None#multiprocessing类是用于多进程
        if num_workers > 1:
        ...
        merge_result(
            Binarizer.binarize(
                ...
                src_with_werdur=args.src_with_werdur,
            )
        )
        if num_workers > 1:
            pool.join()
    ...
    def make_all(lang, vocab):  # vocab为字典，lang为源语言或目标语言
        if args.trainpref:
