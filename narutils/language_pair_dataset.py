...
class LanguagePairDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.src_with_werdur:
            ...
            nodelenth = int(tgt_item[-1])
            treepath = tgt_item[nodelenth+1:-nodelenth]
            assert len(treepath)%nodelenth == 0
            treelenth = int(len(treepath)/nodelenth)
            treepath = treepath.reshape(treelenth,nodelenth)
            #ipdb.set_trace()
            
            pathlength = treepath[-3]
            nodeidx = treepath[-2]
            idxlength = treepath[-1]
            treepath= treepath[:-3]
            # for i in range(len(pathlength)):
            #     if pathlength[i]!=65535:
            #         pathlength= pathlength[i:]
            #         break
            pathlength = pathlength[~np.isin(pathlength, [65535])]
            # for i in range(len(treepath)):
            #     if treepath[i]!=treepath:
            #         treepath= treepath[i:]
            #         break
            treepath = treepath.reshape(-1)
            treepath = treepath[~np.isin(treepath, [65535])]

            #ipdb.set_trace()
            newtreepath = torch.full((len(pathlength),50),65535)
            pathindex = 0
            # print(pathlength)
            for i in range(len(pathlength)):
                newtreepath[i][-1*pathlength[i]:] = treepath[pathindex:pathindex+pathlength[i]]
                pathindex= pathindex+pathlength[i]
            tgt_item = tgt_item[:nodelenth+1]
            # for i in range(len(nodeidx)):
            #     if nodeidx[i]!=65535:
            #         nodeidx= nodeidx[i:]
            #         break
            nodeidx = nodeidx[~np.isin(nodeidx, [65535])]
            # for i in range(len(idxlength)):
            #     if idxlength[i]!=65535:
            #         idxlength= idxlength[i:]
            #         break
            idxlength = idxlength[~np.isin(idxlength, [65535])]
            leafnodeids=[]
            idx = 0            
            for idxl in idxlength:
                ll=[]
                for i in range(idxl):
                    ll.append(int(nodeidx[idx]))
                    idx = idx+1
                leafnodeids.append(ll)
            
            #ipdb.set_trace()
            curr_paths = np.array(newtreepath)
            
            if len(curr_paths[0])>=17:
                l=[i for i in range(len(curr_paths[0])-17)]
                curr_paths=np.delete(curr_paths,l,axis=1)
            else:
                l=[65535 for i in range(len(curr_paths))]
                for i in range(17-len(curr_paths[0])):
                    curr_paths = np.insert(curr_paths, 0,l, axis=1)
            curr_paths[curr_paths==65535]=-1
            #ipdb.set_trace()
            for i in range(len(leafnodeids)):
                if leafnodeids[i] == []:
                    curr_paths = np.delete(curr_paths,i,axis=0)    
                for j in range(len(leafnodeids[i])-1):
                    curr_paths = np.insert(curr_paths,i,curr_paths[i],axis=0)
            ast_code_links = np.zeros((curr_paths.shape[0], curr_paths.shape[0]))
            #ipdb.set_trace()
            for i in range(curr_paths.shape[0]):
                for j in range(curr_paths.shape[0]):
                    farpoint=-1
                    for k in range(-1,-1*curr_paths.shape[1]-1,-1):
                        if curr_paths[i][k]==-1 or curr_paths[j][k]==-1:
                            break
                        if curr_paths[i][k] == curr_paths[j][k]:
                            farpoint = curr_paths[i][k]
                        else:
                            break
                    assert farpoint != -1
                    ast_code_links[i][j] = farpoint
                            
            # for i, l in enumerate(leafnodeids):
            #     ast_code_links[i, l] = 1
            # ast_code_mat = (
            #     np.cumsum(ast_code_links, axis=0) == 1
            # ) * ast_code_links
            ast_paths = torch.tensor(ast_code_links).view(-1)
            
            #ipdb.set_trace()
            if len(tgt_item) != len(for_wer_gather):
                print("id", index)
                print("source", src_item)
                print(tgt_item)
                print(for_wer_gather)
